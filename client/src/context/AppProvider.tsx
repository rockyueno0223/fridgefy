...
export const AppProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addToCart = async (ingredientIds: string[]) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/me/cart/add`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${await getToken()}`,
  ...
  const removeFromCart = async (ingredientIds: string[]) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/me/cart/remove`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${await getToken()}`,
          'Content-Type': 'application/json',
  ...
  const addToFridge = async (ingredientIds: string[]) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/me/fridge/add`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${await getToken()}`,
          'Content-Type': 'application/json',
  ...
  const removeFromFridge = async (ingredientIds: string[]) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/me/fridge/remove`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${await getToken()}`,
          'Content-Type': 'application/json',
  ...
  const checkUniqueFridge = (ingredientIds: string[]): string[] => {
    ...
    const ids = ingredientIds.filter((ingredientId) => !user.fridge.some((item) => item._id === ingredientId));
    return ids;
  };
  ...
  const checkUniqueCart = (ingredientIds: string[]): string[] => {
    ...
    const ids = ingredientIds.filter((ingredientId) => !user.cart.some((item) => item._id === ingredientId));
    return ids;
  };
  ...
  const addToWishlist = async (recipeId: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/me/wishlist/add`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${await getToken()}`,
          'Content-Type': 'application/json',
      ...
      if (data.success) {
        ...
        const addedRecipe = data.wishlist.filter((recipe: IRecipe) => recipe._id === recipeId);
  ...
  const removeFromWishlist = async (recipeId: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/me/wishlist/remove`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${await getToken()}`,
          'Content-Type': 'application/json',
      ...
      if (data.success) {
        ...
        const removedRecipe = wishlist.filter((recipe: IRecipe) => recipe._id === recipeId);
